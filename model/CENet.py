...
class CE_Net_JIAN(nn.Module):
    def __init__(self, n_classes=2, in_channels=3):
        super(CE_Net_JIAN, self).__init__()
...
def show_features(features):
    ...
    if len(features.shape) == 2:
        ...
        for j in range(features.shape[0]//16):
            plt.figure(j)
            for i in range(16):
                plt.subplot(4,4,1+i)
                img = to_pil(features[i+j*16])
                plt.imshow(img)
    plt.show()
...
if __name__ == '__main__':
    ...
    # print(net.encoder1)
    print(logits.shape)
    model = CE_Net_(in_channels=3, n_classes=1)
    model = torch.nn.DataParallel(model).cuda()
    # pretrained_model_path = r'E:\Medical Image Process\Pytorch_Medical_Segmention\Code\checkpoints\Banhen\CE_Net_\CENet(pretrain=True)\model_058_0.7099.pth.tar'
    # checkpoint = torch.load(pretrained_model_path)
    # model.load_state_dict(checkpoint['state_dict'])
    img = Image.open(r'E:\Medical Image Process\Pytorch_Medical_Segmention\Dataset\Banhen\images\1\0.jpg')
    img = np.array(img)
    img = cv2.resize(img,(512,512))
    to_tensor = transforms.ToTensor()
    x = to_tensor(img.copy()).float()
    ...
    # out = out.data.cpu().numpy().squeeze()*255
